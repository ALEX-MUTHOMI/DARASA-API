...
| SEC-P2-014 | Fetch Metadata Request Header Validation | Security | P2 — required before scale optimization | Cross-site request forgery/inclusion attacks that fetch metadata headers can detect may go unflagged. | Middleware | Security Engineer | Security Closeout | A real OWASP ZAP baseline scan (`docs/security/ZAP_DAST_STRATEGY.md`) found the app does not validate `Sec-Fetch-Dest`/`Sec-Fetch-Mode`/`Sec-Fetch-Site` request headers (ZAP rule 90005/90006/90007/90008). Low priority today because the current HTTP surface has no session-cookie-based state-changing endpoint for Fetch Metadata to protect; re-evaluate once Phase 10 introduces authenticated, cookie-or-session-relevant endpoints. |
...
| OBS-P0-003b | Liveness Probe Reliability | Observability | P0 — production blocker | A container orchestrator or uptime monitor cannot know a specific tenant's Host header, so `/api/health/` 404'd for exactly the callers who needed it most. | `core/middleware.py` | Platform Engineer | Observability | **DONE**: `HealthCheckBypassMiddleware` answers `/api/health/` before tenant resolution runs. Verified with a real `docker build`/`docker run`: `docker ps` reports the container `(healthy)`. External uptime/synthetic monitoring against a real deployed URL is still open — see `docs/OBSERVABILITY.md`. |
...
See `docs/security/ZAP_DAST_STRATEGY.md` for the separate concern of dynamic
(runtime) security testing of the application itself via OWASP ZAP,
including why CCT specifically cannot be dynamically tested until Phase 10
gives it HTTP endpoints, and the real, verified findings/fixes that scan
already produced against the current minimal HTTP surface.
