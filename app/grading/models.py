...
class GradeRecord(TimeStampedModel):
    ...
    class Meta:
        ...
        indexes = [
            models.Index(
                fields=["assessment", "subject", "student"],
    ...
    def clean(self):
        ...
        if not (0.0 <= self.raw_score <= 100.0):
            raise ValidationError(
                {"raw_score": _("Raw score must be between 0 and 100.")}
            )
        if not (1 <= self.cbc_score <= 4):
            raise ValidationError(
                {"cbc_score": _("CBC score must be between 1 and 4.")}
            )

    def __str__(self):
        return (
            f"{self.student.admission_number} - "
            f"{self.subject.knec_code}: {self.cbc_score}"
        )
