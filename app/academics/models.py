...
class Cohort(TimeStampedModel):
    """
    Represents a specific Class/Stream in a given Academic Year (e.g. Form 1 East 2026).
    """
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    ...
    is_graduated = models.BooleanField(
        _("Has Graduated?"), 
        default=False,
        help_text=_("Graduated cohorts are read-only to preserve historical integrity.")
    )
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["academic_year", "is_graduated"], name="idx_cohort_year_grad"),
        ]
...
class Student(TimeStampedModel):
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                # Using a generic DB function if supported, or enforcing via clean()
                # We enforce no future DOBs at model clean() since DB functions vary,
                # but let's assume we don't use raw NOW() in constraints for DB agnostic safety.
...
class TeacherAssignment(TimeStampedModel):
    """
    Maps a Teacher (CustomUser in public schema) to a Cohort and Subject in the Tenant Schema.
    """
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    ...
    class Meta:
        ...
        indexes = [
            # High-speed composite index for the Fast-Grid selector
            models.Index(fields=["teacher", "cohort", "subject"], name="idx_fast_grid_auth"),
            models.Index(fields=["cohort", "subject"], name="idx_cohort_subject"),
        ]

    def __str__(self):
        return f"Teacher {self.teacher_id} -> {self.subject.name} in {self.cohort.name}"
